...
const overviewDetailSummary = app.match(/<details class="overview-detail-drawer runtime-drawer">[\s\S]*?<\/summary>/)?.[0] || "";
assert.ok(overviewDetailSummary, "overview runtime drawer summary must remain present");
assert.doesNotMatch(overviewDetailSummary, /:open=/, "overview runtime drawer must default to collapsed");
assert.match(overviewDetailSummary, /<b v-if="aiPaused">AI 已暫停<\/b>/);
assert.match(overviewDetailSummary, /<b v-if="\['running', 'paused'\]\.includes\(aiFailedRetrySweep\.state\)">/);
assert.match(overviewDetailSummary, /<b v-if="redownloadActive">[\s\S]*?redownloadActive\.current[\s\S]*?redownloadActive\.total/);
assert.match(overviewDetailSummary, /<b v-if="recommendations\.length">/);
assert.match(overviewDetailSummary, /<b v-if="sloEvidenceIncomplete" class="slo-incomplete">SLO 證據不完整<\/b>/);
assert.doesNotMatch(overviewDetailSummary, /v-else-if=/, "collapsed summary states must not hide one another");
assert.match(app, /rolling_operational\?\.proof_eligible !== true/);
...
assert.match(app, /const routineDeploymentPause/);
...
const sharedCardRule = css.match(/\.overview-stats article,[\s\S]*?\.tool-result\s*\{[^}]*\}/)?.[0] || "";
assert.ok(sharedCardRule, "shared card rule must remain present");
assert.doesNotMatch(sharedCardRule, /\.page-panel/);
assert.match(css, /\.page-panel\s*\{[^}]*border:\s*0;[^}]*padding:\s*0;[^}]*background:\s*transparent;[^}]*box-shadow:\s*none;/);
